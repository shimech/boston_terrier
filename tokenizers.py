...
import zipfile
import nltk
from sumy._compat import to_string, to_unicode, unicode
from sumy.utils import normalize_language


class SudachiWordTokenizer:
    """
...
class Tokenizer(object):
    ...
    SPECIAL_SENTENCE_TOKENIZERS = {
        'sudachi': nltk.RegexpTokenizer('[^　！？。]*[！？。]'),
    }

    SPECIAL_WORD_TOKENIZERS = {
        'sudachi': SudachiWordTokenizer(),
    }
    ...
    def _get_sentence_tokenizer(self, language):
        ...
        try:
            path = to_string(
                "tokenizers/punkt/%s.pickle") % to_string(language)
